...
class Demo extends React.Component {
    ...
    handleClick2 =(name1,name2,e)=>{
        // 通过bind的方式进行事件函数绑定，合成事件对象在最后一个参数
        console.log(name1,name2,e);
    }
    handleClick3=(e,name1,name2)=>{
        console.log(e,name1,name2);
    }
    render() {
        return <div>
            <button onClick={this.handleClick}>点击我</button>
            {/* 如果需要传参的话第一种方式：可以通过bind */}
            <button onClick={this.handleClick2.bind(this,"张三",'李四')}>按钮2</button>
            {/* 第二种传递一个函数，函数内部写处理函数 */}
            <button onClick={(e)=>{this.handleClick3(e,'张三','李四')}}>按钮3</button>
        </div>
    }
